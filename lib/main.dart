...
class _HomeState extends State<Home> {
  ...
  int _qIdx = 0;
  String _subject = '영어';
  bool _isBack = false, _cmdMode = false;
  int _cardNum = 0, _goodCount = 0;
  bool _soundOn = true, _ttsReady = false;
  ...
  void _load(String s) {
    ...
    if (s == '유머') _queue.addAll(['세상에서가장쉬운AI 이보다더쉬운AI없다','AI먼저말걸면 사람은그냥OX','코딩1도모르고만든앱 그게TikiTaka']);
    else if (s == '뉴스') { _queue.add('로딩...'); _fetchNews(); }
    else if (s == '팔로우') { if(_following.isEmpty)_queue.add('팔로우없음 ▲명령'); else for(final f in _following)_queue.add('⭐$f $f'); }
  ...
  void _ping1() { if(!_soundOn)return; HapticFeedback.lightImpact(); SystemSound.play(SystemSoundType.click); }
  void _ping2() { if(!_soundOn)return; SystemSound.play(SystemSoundType.alert); }
  void _pong() {
    if(!_soundOn)return;
    _ping1();
    Future.delayed(const Duration(milliseconds: 200), _ping2);
  }
  ...
  void _submitText() {
    ...
    else if(r.contains('소리 꺼')){_soundOn=false;_tts.stop();_cmdPending='유머';}
  ...
  @override Widget build(_) => Scaffold(body: SafeArea(child: Stack(children: [
    ...
    // Draggable bar — pan on container, not on buttons
    Positioned(left:_barX+20,bottom:80+_barY,child:GestureDetector(
      ...
      child:Container(
        ...
        child:Row(mainAxisSize:MainAxisSize.min,children:[
          GestureDetector(behavior:HitTestBehavior.opaque,onTap:_onX,child:_btn('✕',Color(0xFF8B4242))),
          SizedBox(width:12),
          GestureDetector(behavior:HitTestBehavior.opaque,onTap:_toggleCmd,child:Container(width:32,height:32,
            decoration:BoxDecoration(shape:BoxShape.circle,color:Colors.white.withAlpha(_cmdMode?15:5)),
            child:Center(child:Text('▲',style:TextStyle(color:_cmdMode?Color(0xFFD4A574):Colors.white24,fontSize:14))))),
          SizedBox(width:12),
          GestureDetector(behavior:HitTestBehavior.opaque,onTap:_onO,child:_btn('○',Color(0xFFD4A574))),
        ])))),
  ])));

  Widget _btn(String s,Color c)=>Container(width:40,height:40,
  ...
  Widget _cmdCard() {
    if(_isBack) return _card('"$_cmdPending"을(를)\n시작합니다.',true);
    return Container(margin:EdgeInsets.symmetric(horizontal:16),padding:EdgeInsets.all(24),
      ...
      child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        Wrap(spacing:8,runSpacing:8,alignment:WrapAlignment.center,
          children:_subjects.map((s)=>ActionChip(label:Text(s,style:TextStyle(color:Colors.white70,fontSize:12)),
